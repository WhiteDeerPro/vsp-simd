...
| encoded MEMORY semantic decode | 已实现固定两 word `VLOAD/VSTORE` profile |
...
immediate body，`SADD` 是单 word；`VLOAD/VSTORE` 使用固定两 word MEMORY record。
MEMORY header 携带 op、address space/context、五位 state base register、VRF row 与
1–16 byte span，第二 word 必须是当前 signed 16-bit memory offset 的 canonical 32-bit
符号扩展。group mask 仍来自 launch envelope，不在 record 中逐项编码。strict wrapper
...
1. 把已经接通的 state/MEMORY strict 路径纳入 ordered action window，并由硬件派生
   state register RAW/WAR/WAW metadata；memory engine 继续只接收已快照的 resolved
   base/offset descriptor。
2. 根据实际多 group 程序加入 per-context `SET_GMASK`，同样在 action admission 时快照。
