...
| Uword program delivery | `vsp_uword_asm_tb.py`、`vsp_uword_bundle_assembler_tb`、`vsp_uword_program_frontend_tb` | 工具对 ALU/local ROUTE/reduction pseudo-op 的精确编码与非法输入拒绝；非零 byte PC、每 word `+4`、完整地址窗口检查、单 outstanding control-store response、MEMORY/EXEC 跨 bundle、record 背压稳定、EOF 截断、空/非法 stream range、live reset 与 fault，共 783 项 frontend 检查；另有 57 项 bundle 连续性、错误流丢弃和交付检查；40-bit PC/3-word bundle 参数 lint | architectural IFetch/ISA、物理 SRAM/I-cache/MMU、branch/loop/redirect、END semantic decode、action envelope、queue/class-router 接线、可持续每拍 4-word/3-action 供给或工具格式长期稳定 |
...
| Strict uword program closure | `vsp_uword_cluster_program_wrapper_tb` | behavioral control store → single-outstanding byte-PC source → 4-word multi-framer → slot-0 action adapter → strict controller 的 EXEC/END 闭环；launch envelope/tag、实际 EXEC result、opaque MEMORY body、ordered MEMORY/CONTROL reject、truncated/empty/missing-END、early/final END、fetch-fault abort/restart，共 148 项检查 | 三项 record 已并发发射；ordered action window 已集成；side 是 scalar ALU；MEMORY uword 能执行 LOAD/STORE；持续每拍 4-word/3-action 供给、branch/loop、真实 I-cache/MMU 或多 active action 已实现 |
