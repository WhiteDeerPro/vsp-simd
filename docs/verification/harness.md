...
| Route rendezvous table | `vsp_route_rendezvous_table_tb` | 3-entry/3-participant 非二次幂配置上，`DEP_IN/DEP_OUT` 按 `{context,epoch,route_id}` 收集，双 role 不同 participant、双 token/payload 保留、单调 frontier 门槛、terminal 背压稳定和 key 占用，越界 context/participant identity、满表背压、ready terminal 连续排出与 RR 跨尾回绕，illegal/同 role/同 participant 冲突 REJECT，冲突 fragment 的 fault identity 保留，exact flush/epoch advance CANCEL、per-context persistent epoch fence、late old-epoch fragment CANCEL、REJECT 优先级和 staged terminal 不受后到 flush 改写，共 131 项检查 | 仅凭该 leaf 测试声称它已连到 predecoder/queue/window/route engine，或声称 frontier 已按 context 维度编号、terminal RR 已保证 architectural issue/retirement order、READY→RUN 资源与双 completion credit 已原子预留、已有 completion fan-out 或顶层 cooperative closure |
| Route-wave controller | `vsp_route_wave_controller_tb` | rendezvous terminal 到 `COLLECT/LAUNCH/RUN/FANOUT` 的控制闭环：两种 half 到达次序、frontier gate、稳定 parent stall、source/destination mask 与 row 合并、单 parent launch、两 participant completion 独立背压、共同 fault 与 destination-only invalid detail、REJECT/CANCEL 零 parent、active-key ABA 阻挡、parent identity mismatch sticky/clear，以及 staged WAVE/LAUNCH 的 flush 与 epoch-advance cancellation（含 terminal 与 flush 同拍、kill 与 ready 同拍时取消优先、零 parent 副作用），共 318 项检查 | 已绑定 PC/queue/window 或真实 frontier；外部 resource grant 已实现物理 group/VRF 仲裁；RUN 中 flush 会回滚 VRF 副作用；已有多个 active parent 或 `II=1` |
| Integrated route-wave execution | `vsp_cluster_route_wave_pipeline_tb` | `DEP_OUT mask=0011` 与 `DEP_IN mask=1100` 在退休 frontier 前零 VRF 副作用，稳定 union-resource grant 与 engine-command 原子 join 后只启动一次真实 register-route；split-mask source/index capture、显式 `ROUTE_RESULT`、write-request stall、masked commit、双 completion 独立背压；第一条 RUN 期间收集第二条完整 WAVE，后者在 staged 状态被 flush，待第一条 FANOUT 清空后输出双 CANCEL，且第二条不申请 resource、不访问 VRF，共 182 项检查 | strict uword/multi-queue program path 已使用该入口；resource-ready 来自真实 scheduler；shared VRF arbiter、engine 多 outstanding、固定总延迟或每拍一条 route 已实现 |
...
| Cluster register route | `vsp_cluster_register_route_engine_tb` | 4×SIMD4/16-byte VRF-indexed gather 在首次 `vd` 写入前完成 source/index row 快照，并经显式 registered `ROUTE_RESULT` stage 保存 data/write mask；覆盖全 mask reverse、稀疏与独立 source/destination mask、单边 empty mask 拒绝、index 指向 source mask 外时 destination undisturbed/invalid、inactive destination、active OOB/缺失 source、mixed validity、`vd==vs/vi` 别名、read completion/response 分离与停顿、read-side 原子 abort、write fault partial commit、`LOCAL/DEP_INOUT`、partial mode reject 和 completion 背压，共 228 项检查 | 当前 engine 为 single-active，经 shared VRF transaction boundary 串行 capture/commit；registered result 不是第二 outstanding slot；strict `DEP_INOUT` 的双槽 dependency precondition 尚无上游证明；指令尚无独立细粒度 predicate，OOB 只提供 no-write/undisturbed sentinel 而不提供 zero-fill；不声称固定总延迟、`II=1`、并行 RF 端口、write-side fault rollback、16-byte 以外 route、scatter/memory gather、最终拓扑或 PPA |
...
当前 route-wave controller 与真实 route engine 的独立 integration test 已覆盖显式
frontier、READY→RUN union-resource handshake、RUN 和双 completion FANOUT；它验证的是
接口合同，不是 program-path 事实。真正接入双槽 program path 时仍需补：`00 LOCAL`
不产生 token；`11` 不等 half peer但等待 participant retirement；真实 ingress/tracker/
reject/completion/MEMORY/VRF 尚未清时不得 READY；等待期间另一槽 older action 可推进；
queue/window retirement 用各 participant 原 sequence 接收 fan-out。当前缺的是这些真实
producer/consumer binding，而不是另一份 route-engine 功能测试。

当前 `make test` 仍是聚合入口；uword 编制器与 program frontend、bundle predecoder、
multi-record framer、ordered action window、route rendezvous/wave pipeline、wrapper、completion tracker、decode holding、result
