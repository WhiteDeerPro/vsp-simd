...
| Cluster register route | `vsp_cluster_register_route_engine_tb` | 4×SIMD4/16-byte VRF-indexed gather 会在首次 `vd` 写入前完成 active group 的 source/index row 快照；全 mask reverse、稀疏 group mask、inactive destination 保持、active OOB/缺失 source 关闭 byte write 并保留 `vd`、同组 mixed-validity 部分写回、全 invalid group 的零 mask no-op write、invalid-element mask 只诊断且不触发 action error、含 mixed-invalid 的 `vd==vs` 以及 `vd==vi` 别名、read completion/response 两种分离次序与停顿、empty-mask 零 VRF 侧效应拒绝、read-side fault 在首次 commit 前原子 abort、write fault 的精确 group mask/partial commit、full-mask `8 read + 4 write` 事务数及 completion 背压稳定，共 161 项检查 | 当前 engine 为 single-active，经 shared VRF transaction boundary 串行 capture/commit；指令尚无独立细粒度 predicate，OOB 只提供 no-write/undisturbed sentinel 而不提供 zero-fill；不声称固定延迟、`II=1`、并行 RF 端口、write-side fault rollback、16-byte 以外 route、scatter/memory gather、最终拓扑或 PPA |
