...
独立 `vsp_vrf_span_engine` 已实现 VRF-only blocking LOAD/STORE parent，
但尚未接入 class router、wrapper、local SRAM 或 DMA。
它传递 effective address、address-space kind 和 address context，但没有
MMU、TLB、PTW 或 cache。当前也没有 architectural IFetch；controller
内部 uword/control-store 交付与未来 IFetch 是不同边界。
...
| `ifetch_*` | 未来 control/instruction fetch 逻辑口；当前没有对应 RTL |
...
- 已有 blocking VRF-only LOAD/STORE 之上的多 outstanding、二维地址、
  物理局部 SRAM、DMA、地址空间/翻译 adapter 与系统集成；
...
transaction wrapper、GROUP_EXEC cluster exec shell、decode holding shell 和独立
VRF-only span engine 已完成参考实现；cluster 默认为四组、两 context、两 slot，
但还不是有状态 controller。当前工作计划继续实现真实 predecode/canonical
expansion、class router、owner/barrier 状态，并把已有 span engine 接到 wrapper 与
local SRAM，形成 VRF LOAD/STORE 数据供应闭环，并根据结果决定 exchange、DMA
