...
跨组 gather 的索引来自 VRF 中的索引向量寄存器，而不是标量控制字：一个 lane 是
8-bit，索引本身占一个 lane，因此索引向量与普通向量寄存器同构，不需要为它在 SRF
中预留超宽控制字。候选 Omega 网络的各级 switch 控制由索引向量经组合逻辑派生，
也不作为架构可见状态保存。
...
复制语义。跨组路由因此改用支持 broadcast 且控制位可由索引位模式直接派生的
Omega 网络，理由见[路由](routing.md)。
