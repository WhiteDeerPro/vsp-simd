...
Bênes 控制字可能超过一个普通标量字，不宜直接假设完整控制字永久占用 SRF。
现有独立 exchange engine 接收外部已经解析的 raw switch control/valid，并在 action
接受时快照；它没有实现 route register 本体。专用表的容量、装载方式和指令编码
仍待设计。
...
对于 `N=2^k、N>=2` 个端口，当前二进制 Bênes 网络包含：

\[
2\log_2N-1
\]

级，每级 `N/2` 个 2×2 switch。它能实现任意排列，但控制字不是简单的“每个输出选择哪个输入”；还需要由目标排列求出合法路由，而且普通交换单元不提供复制语义。因此当前 Bênes RTL 只实现数据网络，路由控制器后续单独研究。
